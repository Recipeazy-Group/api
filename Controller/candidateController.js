...
exports.getCandidateProfile = function(req,res){
    Candidate.getUserByID(req.params.user_id, function(err, candidate){
        if (err){
          res.send(err);
        }
        else{
...
if(!req.params.user_id){
        ...
        Candidate.updateBio(req.params.user_id,req.params.biography,function(err,cand){
            if(err){
                res.send(err);
            }
            else{
...
exports.updateProfilePosition = function(req,res){
    if(!req.params.user_id){
        ...
        Candidate.updatePosition(req.params.user_id,req.params.position,function(err,cand){
            if(err){
                res.send(err);
            }
            else{
...
exports.updateProfileLocation = function(req,res){
    if(!req.params.user_id){
        ...
        Candidate.updateLocation(req.params.user_id,req.params.location,function(err,cand){
            if(err){
                res.send(err);
            }
            else{
